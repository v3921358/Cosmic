...
public class AbstractPlayerInteraction {
    ...
    public void setQuestProgress(int qid, String progress){
        MapleQuestStatus status = c.getPlayer().getQuest(MapleQuest.getInstance(qid));
        status.setProgress(status.getAnyProgressKey(), progress);
        c.getPlayer().announce(MaplePacketCreator.updateQuest(status, false));
    }
    
    public void setQuestProgress(int qid, int pid, String progress) {
        MapleQuestStatus status = c.getPlayer().getQuest(MapleQuest.getInstance(qid));
    ...
    public String getQuestProgressString(int qid){
        MapleQuestStatus qs = c.getPlayer().getQuest(MapleQuest.getInstance(qid));
        String progress = qs.getProgress(qs.getAnyProgressKey());
        return progress.isEmpty() ? "0" : progress;
    }
    
    public String getQuestProgressString (int qid, int pid){
        MapleQuestStatus qs = c.getPlayer().getQuest(MapleQuest.getInstance(qid));
        return qs.getProgress(pid).isEmpty() ? "0" : qs.getProgress(pid);
    }
    
     public int getQuestProgress(int qid) {
        return Integer.parseInt(getQuestProgressString(qid));
    }

    public int getQuestProgress(int qid, int pid) {
        return Integer.parseInt(getQuestProgressString(qid, pid));
    }
