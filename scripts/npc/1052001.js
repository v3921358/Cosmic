...
function action(mode, type, selection) {
    ...
    if (actionx["1stJob"]){
        if (status == 0)
            ...
            if (cm.canHold(2070000) && cm.canHold(1472061)){
                if (cm.getJobId() == 0){
                    ...
                    {
                        cm.getPlayer().teachSkill(14101006, 20, 20, -1);
                    }
                }
        ...
        else if (status == 3)
        {
            if(cm.getPlayer().getMentorId() > 0)
            {
                cm.sendNextPrev("Dig deep into the darkness. Harness the energy as an #bUltimate Explorer#k to reach even greater heights!");    
            }
            else
            {
